...
# Software design
The software is organized around a layered architecture centered on a global controller (*ProgramOrganizer*), which maintains experiment state, configuration parameters, and processing context. This controller can be driven either through the graphical user interface or programmatically, enabling users to visually explore datasets and tune segmentation parameters before reusing the same configurations for headless batch processing. This hybrid GUI/API design enables accessibility and flexibility.
...
Static and temporal analyses are separated through two dedicated classes: *OneImageAnalysis*, responsible for preprocessing tasks such as arena delineation and background subtraction, and *MotionAnalysis*, which performs video-based measurements and temporal feature extraction. This separation allows computationally intensive motion analysis to build upon validated segmentation results.
Cellects targets diverse biological datasets (e.g., fungi, HeLa cells, amoebae) acquired under variable imaging conditions. Rather than relying on a single segmentation model, the image analysis layer provides configurable pipelines combining K-means clustering, threshold-based methods, and multiple color-space representations. Geometric descriptors (area, perimeter, circularity) are encapsulated in the *ShapeDescriptors* class, while additional modules support graph-based dynamics, oscillatory behavior, and morphological operations. This approach avoids specialization to a single organism or imaging modality where the user can create its own customized pipeline.
